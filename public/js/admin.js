...
socket.on("admin_receive_message", (message) => {
    ...
    const divMessages = document.getElementById(`allMessages${message.message.user_id}`);
    const createDiv = document.createElement("div");

    createDiv.className = "admin_message_client";
    createDiv.innerHTML = `<span>${connection.user.email} - ${message.message.text}</span>`;
    createDiv.innerHTML += `<span class="admin_date">${dayjs(message.message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span>`;
